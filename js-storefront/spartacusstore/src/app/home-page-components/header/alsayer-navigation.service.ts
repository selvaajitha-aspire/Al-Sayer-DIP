...
import { AlsayerNavigationNode } from 'src/app/models/alsayer-navigation-node.model';

@Injectable({
    providedIn: 'root'
  })
export class AlsayerNavigationService {
  ...
  ): Observable<AlsayerNavigationNode> {
   ...
   * @param items
   */
  private populateNavigationNode(nodeData: any, items: any): AlsayerNavigationNode {
    const node: AlsayerNavigationNode = {};
   ...
   */
  private populateLink(node: AlsayerNavigationNode, entry, items) {
    const item = items[`${entry.itemId}_${entry.itemSuperType}`];

    // now we only consider CMSLinkComponent
    if (item && (entry.itemType === 'CMSLinkComponent' || entry.itemType === 'SideMenuLinkComponent') ) {
      if (!node.title) {
      ...
      if (node.title && url) {
        ...
        node.media = item.media ? item.media : '';
      }
    }
  }
